...
app_ui = ui.page_sidebar(
    ...
    ui.row(
        ...
        ui.column(6,
            ui.card(ui.card_header("全局特征重要度 (Top-10)"), output_widget("global_imp_plot"))
        )
    ),
    ui.row(
        ui.column(12,
        ...
        ui.column(12,
            ui.card(
                ...
                ui.div(  # ⭐ 用 div 包裹文本+AI解读
                    ui.output_text("pred_text"),
                    ui.output_ui("llm_analysis"),
                    style="padding: 10px;"  # ⭐ 可选，调整上下间距
                )
            )
        )
    )
...
def server(input, output, session):
    ...
    def llm_analysis():
        ...
        prompt = (
            ...
            f"{annual_report}\n"
            f"请结合行业背景、宏观经济环境、以及历史案例，若模型输出为不并购就往不并购方向解析，使用 Markdown 格式输出深度分析报告。"
        )
